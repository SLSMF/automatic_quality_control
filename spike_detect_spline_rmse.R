...
calculate_spikes_via_rmse <- function(data, Nwin=4, ndegree=2, mthd=1, nsigma=4) {
  ...
  spike_flags1 <- spike_detect_spline_rmse(data$slevel,data$time,Nwin,nsigma,ndegree,mthd)
  lg3 <- spike_flags1 == 4 
  ...
  data$slevel[lg3] <- NA
  spike_flags2 <- spike_detect_spline_rmse(data$slevel,data$time,Nwin,nsigma,ndegree,mthd)
  spikes_via_rmse <- spike_flags2 == 4 | spike_flags1 == 4
  # tmp <- length(lg3[lg3==TRUE])
  # cat(tmp," spikes detected after 2 rounds\n")
  # spike_flags <- rep(1,length(data$slevel)) 
  # spike_flags[lg3] <- 4
